...
public class DoacaoController {
    ...
    @ResponseBody
    @PostMapping("/doacao")
    public ResponseEntity realizarDoacao(
            @RequestParam Long idUsuario,
            @RequestParam Long idEntidade,
            @RequestParam Integer valorDoacao) {

        try {
            return ResponseEntity.status(HttpStatus.OK).body(doacaoService.realizarDoacao(idUsuario, idEntidade, valorDoacao));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Erro ao realizar doação!");
        }
